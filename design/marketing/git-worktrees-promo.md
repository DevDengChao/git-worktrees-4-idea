...
**Git Worktrees | GW4I** is a freemium plugin. The core view, filter, sort, search, and open features are free forever. The write operations — checkout, delete, and worktree-aware branch conflict handling — require a GW4I Pro license.

| Feature | Free | Pro |
|---------|------|-----|
| View, filter, sort, search, refresh worktree list | ✅ | ✅ |
| Open worktree as project | ✅ | ✅ |
| Checkout worktree branch | — | ✅ |
| Delete Worktree (single & bulk) | — | ✅ |
| Git Branch / Git Log worktree-aware checkout/delete | — | ✅ |

Start a free trial or purchase GW4I Pro on JetBrains Marketplace (product code `PGWFI`). Licensing is handled entirely by the standard JetBrains Marketplace mechanism — no separate account or server required.

### 中文

**Git Worktrees | GW4I** 是一款 Freemium 插件。查看、过滤、排序、搜索和打开 worktree 永久免费；Checkout、Delete Worktree 以及 Git Branch / Git Log 中处理已被 worktree 占用分支的 checkout/delete 属于 Pro 付费能力。
...
| Delete Worktree（单个和批量） | — | ✅ |
| Git Branch / Git Log worktree-aware checkout/delete | — | ✅ |

在 JetBrains Marketplace 开始免费试用或购买 GW4I Pro（产品码 `PGWFI`）。授权完全通过 JetBrains Marketplace 标准机制处理，无需单独账号或服务器。
